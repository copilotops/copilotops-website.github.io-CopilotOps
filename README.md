...
- **Header** — logo on the left, navigation (including a "Use cases" dropdown) on the right, a
  light/dark theme toggle, collapsing to a menu button below 900px.
...
- **Agent summary & architecture** — six production patterns in a responsive grid (1 column on
  ...
  with a per-build governance rail underneath. Also reachable from the header's "Use cases"
  dropdown.
- **Core capabilities** — the services we offer: Copilot Studio (Standard), Copilot Studio +
  GitHub harness, Cowork (skills, plugins, app building), Governance (Purview, sensitivity
  labels, DLP), Declarative agents (M365 application connections), Scout agent, Voice agent, and
  Power Automation.
- **How we deliver** — four delivery stages.
- **Theme toggle** — a sun/moon button in the header that switches between light and dark, saved
  in the visitor's browser (`localStorage`) and defaulting to their OS preference on first visit.
...
| Core capabilities / services | the `<article class="cap">` blocks in `index.html` |
| "Use cases" dropdown items | the `.nav-dropdown__menu` list in the header of `index.html` |
| Chatbot answers | the `TOPICS` object in `script.js` |
| Which prompts appear first | the `STARTER_PROMPTS` array in `script.js` |
| Light/dark theme colours | the `:root[data-theme="dark"]` block in `style.css` |
